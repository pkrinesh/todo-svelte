<script>
  import { Action, todos, unsubscribe } from '$lib/store/todo';
...
<section class="mt-8 space-y-4">
  {#each $todos as todo (todo.id)}
    {@const completeClass = todo.complete ? 'bg-green-three' : 'bg-leaf-one'}
    ...
    <div class="flex items-center justify-between rounded-md border-2 border-gray-one px-5 py-4">
      <div class="flex w-full items-center justify-start">
        ...
        <input
          ...
          on:change={() => Action.toggle_complete(todo.id)}
        ...
        <input
          ...
          value={todo.text}
          on:input={(e) => Action.edit(todo.id, e.currentTarget.value)}
          class="ml-5 flex-1 text-ellipsis rounded-none border-x-0 border-b border-t-0 border-dashed border-b-gray-two bg-cream-four px-0 pb-1 text-base font-normal text-gray-three placeholder:text-gray-two focus:border-gray-three focus:outline-none focus:ring-0"
        />
        <span
          class="{completeClass} ml-5 hidden rounded-full px-2 py-0.5 text-sm font-normal text-gray-five md:block"
        >
      ...
      <button
        type="button"
        on:click={() => Action.delete(todo.id)}
